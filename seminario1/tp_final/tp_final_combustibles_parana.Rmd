...
Para la comparación de grupos se seleccionaron dos productos comparables dentro del dataset: **Nafta (súper) entre 92 y 95 Ron** y **Nafta (premium) de más de 95 Ron**. La variable cuantitativa analizada fue el `precio`, mientras que la variable de agrupamiento fue el `producto`.

Como se comparan dos grupos independientes y no se asume igualdad de varianzas, se utiliza la **prueba t de Welch**. Esta prueba fue vista en clase y resulta adecuada cuando se desea comparar las medias de una variable cuantitativa entre dos grupos independientes con varianzas distintas o dudosas.

Se plantean las siguientes hipótesis:

- **Hipótesis nula:** el precio promedio de la Nafta (súper) es igual al precio promedio de la Nafta (premium).
- **Hipótesis alternativa:** el precio promedio de la Nafta (súper) es diferente al precio promedio de la Nafta (premium).

```{r comparacion-grupos-welch}
...
# Se verifica que quedaron exactamente dos grupos
levels(datos_naftas$producto)
...
Antes de aplicar la prueba se realiza una revisión de normalidad mediante Shapiro-Wilk y QQ plot. También se compara la dispersión entre ambos grupos mediante una prueba F de igualdad de varianzas.

```{r diagnosticos-naftas}
by(datos_naftas$precio, datos_naftas$producto, shapiro.test)

# Prueba F para comparar varianzas entre los dos grupos
...
ggplot(datos_naftas, aes(sample = precio)) +
  ...
  stat_qq_line() +
  facet_wrap(~ producto) +
  labs(title = "QQ plot del precio por tipo de nafta")
```

Luego se aplica la prueba t de Welch.

```{r prueba-welch-naftas}
...
Si el valor p obtenido es menor a 0,05, se rechaza la hipótesis nula y se concluye que existen diferencias estadísticamente significativas entre los precios promedio de ambos tipos de nafta. Si el valor p es mayor o igual a 0,05, no se rechaza la hipótesis nula y no se cuenta con evidencia suficiente para afirmar que los promedios sean diferentes.
...
La comparación de grupos se realizó mediante una prueba t de Welch para dos grupos independientes, comparando los precios de Nafta (súper) y Nafta (premium). Esta prueba fue seleccionada porque la variable `precio` es cuantitativa, los grupos comparados son independientes y no se asumió igualdad de varianzas. Los modelos lineales permitieron explicar parte de la variabilidad del precio a partir del año y del tipo de producto.
